...
public class ChangeLoginPasswordActivity extends BaseActivity {
    ...
    private void init() {
        ...
        etOldPsw.setKeyListener(DigitsKeyListener.getInstance(digists));//设置限制文本
        etNewPsw.setKeyListener(DigitsKeyListener.getInstance(digists));//设置限制文本
    ...
    private void submit(String oldPsw, final String newPsw) {
        NoGo.create(mActivity)
                .setUrl(ApiUrls.LOGIN.MODIFY_LOGIN_PWD)
                .addParam("oldpwd",oldPsw)
                .addParam("newpwd",newPsw)
                .showDialog()
